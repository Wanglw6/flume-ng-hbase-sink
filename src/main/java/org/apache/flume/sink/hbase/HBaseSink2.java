 ...
 */
public class HBaseSink2 extends AbstractSink implements Configurable {
    private static final Logger logger = LoggerFactory.getLogger(HBaseSink2.class);
    ...
    public HBaseSink2(Configuration conf) {
        this.config = conf;
    }

    @VisibleForTesting
    @InterfaceAudience.Private
    HBaseSink2(Configuration conf, DebugIncrementsCallback cb) {
        this(conf);
        ...
        try {
            ...
            table = runPrivileged(new PrivilegedExceptionAction<HTable>() {
                ...
                public HTable run() throws Exception {
                    if (tableType.equals("tableType")) {
                        HTable table = new HTable(config, GetTableName.getTableName(dbUrl, userName, password, tableName, sql, monitorUrl));
                        table.setWriteBufferSize(5242880);
                        table.setAutoFlush(false);
                        return table;
                    } else {
...
/*    if (StringUtils.isNotBlank(context.getString(DBURL))) {
    ...
    public Status process() throws EventDeliveryException {
        ...
        try {
            ...
            logger.info("===Sink {} ,get events , {} ms", getName(), endTime - startTime);
